...
type TeamQuery struct {
	...
	// eager-loading edges.
	withCreateUser *UserQuery
	withTeamUsers  *TeamUserQuery
...
// QueryCreateUser chains the current query on the "create_user" edge.
func (tq *TeamQuery) QueryCreateUser() *UserQuery {
...
func (tq *TeamQuery) QueryTeamUsers() *TeamUserQuery {
	query := &TeamUserQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withCreateUser: tq.withCreateUser.Clone(),
		withTeamUsers:  tq.withTeamUsers.Clone(),
...
// WithCreateUser tells the query-builder to eager-load the nodes that are connected to
// the "create_user" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithCreateUser(opts ...func(*UserQuery)) *TeamQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withCreateUser = query
	return tq
}
...
func (tq *TeamQuery) WithTeamUsers(opts ...func(*TeamUserQuery)) *TeamQuery {
	query := &TeamUserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withTeamUsers = query
	return tq
}
...
func (tq *TeamQuery) sqlAll(ctx context.Context) ([]*Team, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [2]bool{
			tq.withCreateUser != nil,
			tq.withTeamUsers != nil,
		}
	)
	...
	if query := tq.withCreateUser; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.CreateUser = n
			}
		}
	}

	if query := tq.withTeamUsers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Team)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.TeamUsers = []*TeamUser{}
		}
		query.Where(predicate.TeamUser(func(s *sql.Selector) {
			s.Where(sql.InValues(team.TeamUsersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.TeamUsers = append(node.Edges.TeamUsers, n)
		}
	}
