...
func (uq *UserQuery) QueryTeamUsers() *TeamUserQuery {
	query := &TeamUserQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithTeamUsers(opts ...func(*TeamUserQuery)) *UserQuery {
	query := &TeamUserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withTeams; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Teams = []*Team{}
		}
		query.Where(predicate.Team(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TeamsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "create_user_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.Teams = append(node.Edges.Teams, n)
		}
	}
