...
type TeamUserFunc func(context.Context, *ent.TeamUserMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TeamUserFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TeamUserMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TeamUserMutation", m)
	}
	return f(ctx, mv)
}
